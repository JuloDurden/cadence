...
- ~~**Tailles verrouillées + 2 zones Sprint/Produit**~~ — fait en v0.97.1 (2026-08-03), suite Phase 4 (retour Julien après le 1er essai). Widgets à valeur unique verrouillés sur une seule taille ; 2 zones séparées (Sprint en cours / Vue produit) selon un `scope` par widget, orientation et partage d'espace réglables via une vraie poignée à glisser par paliers. Corrigé au passage : "Blocages actifs" scopé au jour, contenu débordant d'une tuile. Voir `DashboardZoneSplit.tsx`.
- ~~**5 tailles normalisées (S/M/L/XL/XL Portrait)**~~ — fait en v0.97.2 (2026-08-03), suite Phase 4 (retour Julien : "il faut aussi retravailler et normaliser les tailles"). Reprise sur le modèle des widgets iOS, chaque taille un multiple explicite de S : M (2S × 1S), L (2S × 2S), XL (4S × 2S, ex. Burndown), XL Portrait (2S × 4S, ex. Santé clients — liste verticale dense). Une zone qui rétrécit ne redimensionne jamais les widgets, seule sa grille se recompose. Voir `data/dashboardWidgets.ts`.
- ~~**Face cachée de réglages par widget**~~ — fait en v0.97.3 (2026-08-06), suite Phase 4 (retour Julien : "on va développer d'autres widgets aujourd'hui donc on va rapidement le propager"). Retournement 3D façon iOS (`FlipCard.tsx`, composant générique front/back), bouton Réglages en haut-droite visible en mode "Personnaliser". Câblé pour l'instant uniquement sur "Sprint actuel" (`SprintProgressCard.tsx`) : choix de la métrique mise en avant, SP fait ou % complété, persisté par widget (`DashboardWidgetPlacement.emphasis`). À propager aux prochains widgets développés — l'API (front/back) ne devrait pas avoir besoin de changer, seul le contenu de la face cachée est propre à chaque widget.
...
- **Réforme du Changelog : DB + outil de publication + heatmap façon GitHub + nav groupée + pagination** ✅ *v0.98.21*, 2026-08-22, retour Julien : l'historique (170 versions) devenait ingérable en dur dans `data/changelog.ts` (1400+ lignes), et la mise en page (heatmap en barres, nav plate) ne suivait plus. Migré vers une table dédiée (`changelog_entries`), publiée depuis une modale réservée Admin (`POST /api/changelog`) plutôt qu'en éditant du code - historique existant importé en base par script one-shot. Heatmap redessinée façon contribution graph GitHub (grille semaine/jour) ; nav et liste regroupent désormais les versions mineures consécutives d'un même préfixe en un seul bloc repliable ; pagination par groupes plutôt que tout charger d'un coup. Voir `docs/corrections.md`.
