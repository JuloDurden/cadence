...
export function DashboardPage() {
  ...
  const doneItems = useMemo(() => state.items.filter(i => ['done', 'delivered'].includes(i.status)), [state.items])

  const avgVelocity = useMemo(() => {
    const vels = closedSprints.map(s => s.velocitySnapshot ?? state.items.filter(i => i.sprintId === s.id && ['done','delivered'].includes(i.status)).reduce((sum, i) => sum + i.sp, 0))
    return vels.length ? Math.round(vels.reduce((a, b) => a + b, 0) / vels.length) : 0
  }, [closedSprints, state.items])

  const currentItems = currentSprint ? state.items.filter(i => i.sprintId === currentSprint.id) : []
  const currentDoneSP = currentItems.filter(i => ['done','delivered'].includes(i.status)).reduce((s, i) => s + i.sp, 0)
    ...
    <>
      ...
      <div className="page-content">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
          <VelocityChart sprints={state.sprints} items={state.items} />
          {currentSprint
            ? <BurndownChart sprint={currentSprint} items={state.items} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 16 }}>
          <ClientRAG clients={state.clients} items={state.items} />
