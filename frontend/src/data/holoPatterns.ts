...
// tout est transparent) sont masquees par CANAL ALPHA, comportement par defaut de mask-image.
// 'scanlines' est un degrade CSS avec de vraies zones `transparent` : alpha aussi. 'waves' est un
// degrade CSS purement colore (pas de transparence propre sur sa 2e couche) - masque par
// LUMINANCE (mask-mode: luminance) : la clarte de chaque couleur sert d'alpha de masquage a la
// place du canal alpha reel (quasi toujours a 1 pour un gradient CSS classique), ce qui recree
// tout de meme un motif visible plutot qu'un cache uniformement opaque.
//
// Chaque motif porte donc sa PROPRE taille/repetition/position de masque (mask-size/-repeat/
// -position), pas une valeur unique figee pour tous : leurs tuiles natives n'ont pas le meme
// format (carre pour 'diamonds'/'geometric', rectangulaire pour 'hexagons'/'overlook'/'bubbles',
// degrades auto-repetitifs sans tuile explicite pour 'scanlines'/'ripple'/'waves'). Registre
// volontairement extensible (Julien fournira d'autres motifs plus tard) : il suffit d'ajouter une
// entree ici, aucune autre modification necessaire cote Reglages ou CSS (voir
// getHoloPatternMaskCss ci-dessous, consommee via des variables CSS --hc-pattern-mask-* posees en
// JS, voir PersonalSettingsContext.tsx et AppearanceSection.tsx).

interface HoloPatternDef {
  id: HoloPatternId
  label: string
  /** Valeur CSS mask-image complete (peut comporter plusieurs couches separees par des virgules,
   *  comme un background-image a plusieurs couches). */
  maskImage: string
  maskSize: string
  maskRepeat: string
  maskPosition: string
  /** 'luminance' pour les motifs CSS en couleur sans transparence propre (voir commentaire de
   *  fichier ci-dessus) ; omis (= 'alpha', comportement par defaut de mask-image) pour les
   *  silhouettes SVG deja transparentes en dehors de leur trace. */
  maskMode?: 'alpha' | 'luminance'
}

/** Construit un data-URI SVG minimal (un seul <path>, plein ou au trait) utilisable comme
 *  mask-image : seule la silhouette compte (un masque n'utilise que la transparence/luminance du
 *  rendu, jamais sa couleur), donc toujours rendue en noir plein ici. `mode: 'stroke'` pour un
 *  motif dessine au trait (Hexagones/Overlook, fournis par Julien avec un simple contour, fond
 *  transparent en dehors), `mode: 'fill'` pour un motif en silhouette pleine (Losanges/Motifs
 *  geometriques). */
function svgMaskUrl(pathD: string, width: number, height: number, mode: 'fill' | 'stroke'): string {
  const shape = mode === 'fill'
    ? `<path d='${pathD}' fill='#000'/>`
    : `<path d='${pathD}' fill='none' stroke='#000' stroke-width='1' stroke-linecap='square'/>`
...
/** Jeu complet de proprietes CSS de masquage pour le motif choisi (mask-image/-size/-repeat/
 *  -position/-mode) ; repli sur 'diamonds' si un id inconnu/plus au registre est stocke en base
 *  (voir PersonalSettingsContext.tsx, qui pose ces 5 valeurs en variables CSS globales, et
 *  AppearanceSection.tsx, qui les surcharge localement par vignette dans le selecteur de motif). */
export function getHoloPatternMaskCss(id: HoloPatternId | undefined): HoloPatternMaskCss {
  const def = HOLO_PATTERNS.find(p => p.id === id) ?? HOLO_PATTERNS.find(p => p.id === DEFAULT_HOLO_PATTERN)!
  return { image: def.maskImage, size: def.maskSize, repeat: def.maskRepeat, position: def.maskPosition, mode: def.maskMode ?? 'alpha' }
}
