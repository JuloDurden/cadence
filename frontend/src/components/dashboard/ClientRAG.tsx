...
interface Props { clients: Client[]; items: Item[] }

export function ClientRAG({ clients, items }: Props) {
  return (
    <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow)', padding: '18px 20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {clients.map(client => {
          ...
          const done = clientItems.filter(i => ['done', 'delivered'].includes(i.status)).length
